...
for year in range(year_start, year_end + 1):
    ...
    for month in months:
        ...
        #                    R9 = R8 + 1
			...
			#hist_tt = tuple(hist_total)
# for different k (this case, we knew k=3)
k = 6
n = 0       # control iteration numberi
#num = hist_total.shape[0]
#num = hist_total[0]
num = len(hist_total)
#num = hist_tt.shape[0]
#print num
...
#print center
center[:,:]= random.sample(hist_total, k)
#print center
# judge to run
changed = True
while changed:
    ...
    # geometrical center  
    for i in range(k):
        #new_centers[i,:] = np.mean(hist_total[labels == i,:],axis=0)
        new_centers[i,:] = np.mean(hist_total[0:10],axis=0)
	#new_centers[i,:] = np.mean(hist_total[labels == i],axis=0)
        n = n+1
        # judgement for cycle
