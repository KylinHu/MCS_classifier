...
#num = hist_total[0]
#num = len(hist_total)
num,dim = hist_tt.shape
print num,dim
# sample,initial center
import random
center = np.zeros([k,42])
#print center
center[:,:]= random.sample(hist_tt, k)
#print center
# judge to run
changed = True
while changed:
    ...
    # geometrical center  
    for i in range(k):
        new_centers[i,:] = np.mean(hist_tt[labels == i,:],axis=0)
        #new_centers[i,:] = np.mean(hist_total[labels == i,:])
        n = n+1
        # judgement for cycle
